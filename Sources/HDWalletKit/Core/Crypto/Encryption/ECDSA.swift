...
public final class ECDSA {
    ...
    public func verifySignature(_ sigData: Data, message: Data, publicKeyData: Data) throws -> Bool {
        ...
        secp256k1_ecdsa_signature_parse_der(ctx, &signature, Array(sigData), sigData.count)
        
        if (secp256k1_ec_pubkey_parse(ctx, &pubkey, Array(publicKeyData), publicKeyData.count) != 1) {
            return false
        };
        
        if (secp256k1_ecdsa_verify(ctx, &signature, Array(message), &pubkey) != 1) {
            return false
        };
